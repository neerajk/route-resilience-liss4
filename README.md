...
## Three models you can run (new here? start with this)
All three are road segmenters that output a `pred_mask.tif` for Phase 2. Same training CLI —
only the `--config` changes. Phase-1 code lives under `src/phase1/{shared,vista,grove,vista_v2}/`.

| model | what it is (plain words) | run it |
|---|---|---|
| **VISTA** (baseline) | classifies each pixel road/not-road from its appearance | `python -m src.phase1.vista.train --config config/phase1/config.yaml` |
| **GROVE** (Arm B) | recovers roads **hidden under canopy** via continuity/topology (structure-aware) | `python -m src.phase1.grove.train --config config/phase1/grove.yaml` |
| **VISTA-v2** | ResNet-101 + UNet++ that **benchmarks 3 positional encodings** (botnet/rope/sincos) | `python -m src.phase1.vista.train --config config/phase1/vista_v2_botnet.yaml` |

New to the repo? Read in this order: this table → [`docs/vista_v2.md`](docs/vista_v2.md)
(VISTA-v2 from scratch) → "Two arms" below (VISTA vs GROVE) → [`RUNBOOK.md`](RUNBOOK.md) (every command).
...
```bash
python -m src.phase1.vista.pretrain --config config/phase1/vista_v2_pretrain.yaml   # shared DeepGlobe pretrain
python -m src.phase1.vista.train    --config config/phase1/vista_v2_botnet.yaml      # | rope | sincos | nope
